...
static int copy_from_nullb(struct nullb *nullb, struct page *dest,
	unsigned int off, sector_t sector, size_t n, struct rock_rq *cur_rock)
{
	size_t temp, count = 0;
	unsigned int offset;
	struct nullb_page *t_page;
...
static bool rocks_rb_tree_search(struct rb_root *root, unsigned int offset)
{
	...
	while (node) {
		struct rock_rb_node *data = container_of(node, struct rock_rb_node, node);
		int result;
...
static bool rocks_rb_tree_insert(struct rb_root *root, struct rock_rb_node *data)
{
	struct rb_node **new = &(root->rb_node), *parent = NULL;

	while (*new) {
		struct rock_rb_node *this = container_of(*new, struct rock_rb_node, node);
...
static int null_transfer(struct nullb *nullb, struct page *page,
	...
	bool is_fua, struct rock_rq *cur_rock)
{
	struct nullb_device *dev = nullb->dev;
...
static int null_handle_rq(struct nullb_cmd *cmd)
{
	...
	struct bio_vec bvec;
	struct rock_rq *cur_rock = kcalloc(1, sizeof(struct rock_rq), GFP_KERNEL);
	...
	if (op_is_write(req_op(rq))
			&& (zone->type != BLK_ZONE_TYPE_CONVENTIONAL
				&& cur_rock->dio_tag == true)) {
		struct rock_rb_node *temp_rock_node = kmalloc(sizeof(struct rock_rb_node),
				GFP_KERNEL);
		temp_rock_node->length = cmd->rq->__data_len;
		temp_rock_node->offset = rq->bio->rock_addr;
		if (!rocks_rb_tree_insert(&rock_tree_root, temp_rock_node))
